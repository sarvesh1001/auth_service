...
// GetTimeBucket returns time bucket number (NOT bucket start timestamp)
// For a 3600-second window, this returns the bucket number (e.g., 489196)
// NOT the bucket start timestamp (e.g., 1761105600)
func (bm *BucketingManager) GetTimeBucket(windowSeconds int) int64 {
    return time.Now().Unix() / int64(windowSeconds)
}

// GetTimeBucketStart returns the start timestamp of the current time bucket
// For a 3600-second window, this returns the bucket start (e.g., 1761105600)
func (bm *BucketingManager) GetTimeBucketStart(windowSeconds int) int64 {
    return time.Now().Unix() / int64(windowSeconds) * int64(windowSeconds)
}

// GetTimeBucketForTime returns the bucket number for a specific time
func (bm *BucketingManager) GetTimeBucketForTime(t time.Time, windowSeconds int) int64 {
    return t.Unix() / int64(windowSeconds)
}
