...
// AttendanceResolutionHandler handles attendance resolution endpoints.
type AttendanceResolutionHandler struct {
	resolutionService resolution.ResolutionService
	logger            *zap.Logger
}

// NewAttendanceResolutionHandler creates a new handler.
...
type ResolveDayRequest struct {
	...
	SubjectType string    `json:"subject_type,omitempty"` // added – defaults to "employee"
}
...
func (h *AttendanceResolutionHandler) ResolveDay(w http.ResponseWriter, r *http.Request) {
	...
	if req.Recalculate {
		if err := h.resolutionService.RecalculateDay(ctx, companyID, req.UserID, subjectType, req.Date); err != nil {
	...
	if err := h.resolutionService.ResolveDay(ctx, companyID, req.UserID, subjectType, req.Date); err != nil {
...
func (h *AttendanceResolutionHandler) BatchResolve(w http.ResponseWriter, r *http.Request) {
	...
	h.respondWithJSON(w, http.StatusOK, map[string]interface{}{
		...
		"data":    map[string]interface{}{"processed_count": len(req.EventIDs)},
	})
}
...
func (h *AttendanceResolutionHandler) ResolveDayByPath(w http.ResponseWriter, r *http.Request) {
	...
	// Read subject_type from query, default to "employee"
	...
	if recalculate {
		if err := h.resolutionService.RecalculateDay(ctx, companyID, userID, subjectType, date); err != nil {
	...
	if err := h.resolutionService.ResolveDay(ctx, companyID, userID, subjectType, date); err != nil {
...
func (h *AttendanceResolutionHandler) BatchResolveByPeriod(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	companyID, err := getCompanyIDFromContext(ctx)
	if err != nil {
		h.respondWithError(w, http.StatusUnauthorized, err.Error())
	...
	if err := h.resolutionService.ResolvePeriod(ctx, companyID, req.UserIDs, subjectType, req.StartDate, req.EndDate, req.Recalculate); err != nil {
		h.logger.Error("Failed to resolve period",
