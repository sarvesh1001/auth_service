...
func (h *OrderHandler) injectIdempotencyKey(ctx context.Context, r *http.Request) context.Context {
...
// ------------------- MUTATION ENDPOINTS (with idempotency context injection) -------------------
...
func (h *OrderHandler) CreateDraftOrder(w http.ResponseWriter, r *http.Request) {
	...
	// inject into context for any service method that might read it
...
// ------------------- READ-ONLY ENDPOINTS (no idempotency injection needed) -------------------
...
func (h *OrderHandler) ListOrders(w http.ResponseWriter, r *http.Request) {
	...
	filter := service.OrderListFilter{CompanyID: companyID}
...
func (h *OrderHandler) PreviewPricing(w http.ResponseWriter, r *http.Request) {
	// preview is read‑only, but we still validate permissions
	ctx := r.Context()
	userID, err := h.getUserIDFromContext(ctx)
	if err != nil {
		h.respondWithError(w, http.StatusUnauthorized, "authentication required")
	...
	for i, it := range req.Items {
		...
		items[i] = previewItem{ProductID: productID, Quantity: quantity}
	}
...
func (h *OrderHandler) GetOrderRevenue(w http.ResponseWriter, r *http.Request) {
	...
	h.respondWithJSON(w, http.StatusOK, map[string]interface{}{
		"success": true,
		"data":    map[string]string{"revenue": revenue.String()},
	})
}
...
func (h *OrderHandler) GetAverageOrderValue(w http.ResponseWriter, r *http.Request) {
	...
	h.respondWithJSON(w, http.StatusOK, map[string]interface{}{
		"success": true,
		"data":    map[string]string{"average_order_value": avg.String()},
	})
}
