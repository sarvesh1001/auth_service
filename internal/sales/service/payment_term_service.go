...
// PaymentTermService defines the interface (with idempotency keys added for consistency)
type PaymentTermService interface {
	CreatePaymentTerm(ctx context.Context, req *CreatePaymentTermRequest, idempotencyKey string) (*models.PaymentTerm, error)
	UpdatePaymentTerm(ctx context.Context, companyID uuid.UUID, termID uuid.UUID, req *UpdatePaymentTermRequest, idempotencyKey string) (*models.PaymentTerm, error)
	DeletePaymentTerm(ctx context.Context, companyID uuid.UUID, termID uuid.UUID, deletedBy uuid.UUID, idempotencyKey string) error
	...
	ActivatePaymentTerm(ctx context.Context, companyID uuid.UUID, termID uuid.UUID, updatedBy uuid.UUID, idempotencyKey string) error
	DeactivatePaymentTerm(ctx context.Context, companyID uuid.UUID, termID uuid.UUID, updatedBy uuid.UUID, idempotencyKey string) error
	...
	CalculateEarlyPaymentDiscount(ctx context.Context, companyID uuid.UUID, termID uuid.UUID, invoiceAmount decimal.Decimal, paymentDate time.Time, invoiceDate time.Time) (decimal.Decimal, error)
	AssignPaymentTermToCustomer(ctx context.Context, companyID uuid.UUID, customerID uuid.UUID, termID uuid.UUID, updatedBy uuid.UUID, idempotencyKey string) error
	RemovePaymentTermFromCustomer(ctx context.Context, companyID uuid.UUID, customerID uuid.UUID, updatedBy uuid.UUID, idempotencyKey string) error
...
// CreatePaymentTerm
// ------------------------------
func (s *paymentTermService) CreatePaymentTerm(ctx context.Context, req *CreatePaymentTermRequest, idempotencyKey string) (*models.PaymentTerm, error) {
	logger := s.logger.With(zap.String("method", "CreatePaymentTerm"), zap.String("idempotency_key", idempotencyKey))
	...
	if s.auditService != nil {
		_ = s.auditService.LogAction(ctx, nil, &req.CompanyID, "sales", "create_payment_term", "payment_term",
			&term.TermID, "user", req.CreatedBy, nil, nil, map[string]interface{}{
				"code": term.Code,
				"name": term.TermName,
			})
	}
...
// UpdatePaymentTerm
// ------------------------------
func (s *paymentTermService) UpdatePaymentTerm(ctx context.Context, companyID uuid.UUID, termID uuid.UUID, req *UpdatePaymentTermRequest, idempotencyKey string) (*models.PaymentTerm, error) {
	logger := s.logger.With(zap.String("method", "UpdatePaymentTerm"), zap.String("idempotency_key", idempotencyKey))
	...
	if s.auditService != nil {
		_ = s.auditService.LogAction(ctx, nil, &companyID, "sales", "update_payment_term", "payment_term",
			&termID, "user", req.UpdatedBy, nil, nil, changes)
	}
...
// ------------------------------
// DeletePaymentTerm (soft delete or deactivate if used)
// ------------------------------
func (s *paymentTermService) DeletePaymentTerm(ctx context.Context, companyID uuid.UUID, termID uuid.UUID, deletedBy uuid.UUID, idempotencyKey string) error {
	logger := s.logger.With(zap.String("method", "DeletePaymentTerm"), zap.String("idempotency_key", idempotencyKey))
	...
	if s.auditService != nil {
		_ = s.auditService.LogAction(ctx, nil, &companyID, "sales", "delete_payment_term", "payment_term",
			&termID, "user", &deletedBy, nil, nil, nil)
	}
...
func (s *paymentTermService) GetPaymentTermByID(ctx context.Context, companyID uuid.UUID, termID uuid.UUID) (*models.PaymentTerm, error) {
	return s.repo.GetByID(ctx, nil, companyID, termID)
}
...
func (s *paymentTermService) GetPaymentTermByCode(ctx context.Context, companyID uuid.UUID, code string) (*models.PaymentTerm, error) {
	return s.repo.GetByCode(ctx, nil, companyID, code)
}
...
func (s *paymentTermService) GetPaymentTermByName(ctx context.Context, companyID uuid.UUID, termName string) (*models.PaymentTerm, error) {
	return s.repo.GetByName(ctx, nil, companyID, termName)
}
...
func (s *paymentTermService) ListPaymentTerms(ctx context.Context, filter PaymentTermListFilter, p Pagination, srt Sort) ([]*models.PaymentTerm, int64, error) {
	repoFilter := repository.PaymentTermFilter{
		...
		// Search is intentionally omitted – use SearchPaymentTerms() for text search
	}
	return s.repo.List(ctx, nil, repoFilter,
...
func (s *paymentTermService) SearchPaymentTerms(ctx context.Context, companyID uuid.UUID, query string, limit int, offset int) ([]*models.PaymentTerm, int64, error) {
	return s.repo.Search(ctx, nil, companyID, query, limit, offset)
}

func (s *paymentTermService) GetActivePaymentTerms(ctx context.Context, companyID uuid.UUID) ([]*models.PaymentTerm, error) {
	return s.repo.GetActiveTerms(ctx, nil, companyID)
}

// ------------------------------
// Activate / Deactivate
// ------------------------------
func (s *paymentTermService) ActivatePaymentTerm(ctx context.Context, companyID uuid.UUID, termID uuid.UUID, updatedBy uuid.UUID, idempotencyKey string) error {
	return s.setActiveStatus(ctx, companyID, termID, true, updatedBy, idempotencyKey, salesEvents.EventPaymentTermActivated)
}

func (s *paymentTermService) DeactivatePaymentTerm(ctx context.Context, companyID uuid.UUID, termID uuid.UUID, updatedBy uuid.UUID, idempotencyKey string) error {
	return s.setActiveStatus(ctx, companyID, termID, false, updatedBy, idempotencyKey, salesEvents.EventPaymentTermDeactivated)
}

func (s *paymentTermService) setActiveStatus(ctx context.Context, companyID, termID uuid.UUID, active bool, updatedBy uuid.UUID, idempotencyKey, eventType string) error {
	tx, err := s.pgClient.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("begin tx: %w", err)
	}
	...
	if s.auditService != nil {
		...
		_ = s.auditService.LogAction(ctx, nil, &companyID, "sales", action, "payment_term",
			&termID, "user", &updatedBy, nil, nil, nil)
	}
	return nil
}

func (s *paymentTermService) IsPaymentTermActive(ctx context.Context, companyID uuid.UUID, termID uuid.UUID) (bool, error) {
	return s.repo.IsActive(ctx, nil, companyID, termID)
}
...
func (s *paymentTermService) CalculateDueDate(ctx context.Context, companyID uuid.UUID, termID uuid.UUID, invoiceDate time.Time) (time.Time, error) {
	return s.repo.CalculateDueDate(ctx, nil, companyID, termID, invoiceDate)
}
...
func (s *paymentTermService) CalculateEarlyPaymentDiscount(ctx context.Context, companyID uuid.UUID, termID uuid.UUID, invoiceAmount decimal.Decimal, paymentDate, invoiceDate time.Time) (decimal.Decimal, error) {
	return s.repo.CalculateEarlyPaymentDiscount(ctx, nil, companyID, termID, invoiceAmount, paymentDate, invoiceDate)
}

// ------------------------------
// Customer assignment
// ------------------------------
func (s *paymentTermService) AssignPaymentTermToCustomer(ctx context.Context, companyID uuid.UUID, customerID uuid.UUID, termID uuid.UUID, updatedBy uuid.UUID, idempotencyKey string) error {
	tx, err := s.pgClient.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("begin tx: %w", err)
	}
	...
	if s.auditService != nil {
		_ = s.auditService.LogAction(ctx, nil, &companyID, "sales", "assign_payment_term", "customer",
			&customerID, "user", &updatedBy, nil, nil, map[string]interface{}{
...
func (s *paymentTermService) RemovePaymentTermFromCustomer(ctx context.Context, companyID uuid.UUID, customerID uuid.UUID, updatedBy uuid.UUID, idempotencyKey string) error {
	tx, err := s.pgClient.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("begin tx: %w", err)
	}
	...
	if s.auditService != nil {
		_ = s.auditService.LogAction(ctx, nil, &companyID, "sales", "remove_payment_term", "customer",
			&customerID, "user", &updatedBy, nil, nil, nil)
	}
	return nil
}
