...
func (s *PairingService) ConfirmPairing(ctx context.Context, sessionID string) (*models.TokenPairResponse, error) {
	...
	// ✅ CRITICAL FIX: Normalise mask to exactly 13 blocks (800 permissions)
...
// ✅ FIXED: Always returns 13 blocks (for 800 permissions).
// 🚨 SECURITY NOTE: For production, do NOT trust client‑supplied permissions.
// Instead, fetch the actual permissions from the database using the role/user ID.
