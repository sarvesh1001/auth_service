...
// Note: This implementation is IMMUTABLE - no UPDATE or DELETE operations
...
func (r *AuditRepositoryImpl) CreateAuditLog(ctx context.Context, log *AuditLog) error {
	...
	// Convert []byte to sql.NullBytes for proper NULL handling
	...
	if err != nil {
		r.logger.Error("Failed to create audit log",
			...
			util.String("entity_type", log.EntityType),
			util.ErrorField(err))
...
func (r *AuditRepositoryImpl) CreateAuditLogBatch(ctx context.Context, logs []*AuditLog) error {
	...
	r.logger.Info("Batch audit logs creation completed",
		util.Int("logs_created", len(logs)))

	return nil
}

// ============================================================================
// READ OPERATIONS (STORAGE-ONLY)
// ============================================================================
...
func (r *AuditRepositoryImpl) ListAuditLogs(
	...
	// Helper to add condition with pointer check
...
func (r *AuditRepositoryImpl) HealthCheck(ctx context.Context) error {
	query := `SELECT 1 FROM audit.audit_logs LIMIT 1`
	_, err := r.client.Exec(ctx, query)
	if err != nil {
...
func (r *AuditRepositoryImpl) scanAuditLog(rows *sql.Rows) (*AuditLog, error) {
	...
	// Copy byte slices (they're safe to use directly)
	if beforeState != nil {
...
// parseNullableUUID converts sql.NullString to *uuid.UUID
...
// initializePreparedStatements creates prepared statements for frequently used queries
