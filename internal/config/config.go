...
type Config struct {
	...
	Postgres      PostgresConfig // ✅ ADD THIS
...
func LoadConfig() *Config {
	once.Do(func() {
		...
		cfg = &Config{
			...
			JWT:  loadJWTConfig(environment), // ✅ JWT CONFIG LOADED HERE
			Encryption: EncryptionConfig{
...
// ✅ NEW FUNCTION: loadJWTConfig loads JWT-specific configuration
func loadJWTConfig(env string) JWTConfig {
	...
	return JWTConfig{
		...
		RefreshTTL:          getEnvAsDuration("JWT_REFRESH_TTL", 168*time.Hour), // 7 days
		SigningMethod:       getEnv("JWT_SIGNING_METHOD", "HS256"),
		RotateRefreshTokens: getEnvAsBool("JWT_ROTATE_REFRESH_TOKENS", !isDev), // true in production
	}
}
...
// ✅ UPDATED validateConfig with JWT validation
func validateConfig(cfg *Config) {
	if cfg.Environment == "production" {
		...
		// ✅ NEW: Validate JWT configuration for production
		...
		if cfg.JWT.AccessTTL > 30*time.Minute {
			util.Warn("JWT_ACCESS_TTL is longer than 30 minutes - consider reducing for better security")
		}
		if cfg.JWT.RefreshTTL > 30*24*time.Hour {
			util.Warn("JWT_REFRESH_TTL is longer than 30 days - consider reducing for better security")
		}
		if cfg.JWT.SigningMethod != "HS256" && cfg.JWT.SigningMethod != "RS256" {
			util.Warn("JWT_SIGNING_METHOD should be either HS256 or RS256")
		}
		if !cfg.JWT.RotateRefreshTokens {
			util.Warn("JWT_ROTATE_REFRESH_TOKENS is disabled - this reduces security against token replay attacks")
		}
