// internal/hr/repository/attendance/postgresql.go
package repository
...
func buildNamedQueryArgs(event *attendance.AttendanceEvent) (string, []interface{}) {
	...
	metadataJSON, _ := json.Marshal(event.Metadata) // ✅ ALWAYS marshal
...
func (r *attendanceRepository) CreateBulkAttendanceEvents(
	...
	for _, event := range events {
		...
		metadataJSON, _ := json.Marshal(event.Metadata) // ✅ FIX
...
func (r *attendanceRepository) CreateAttendanceDailySummary(
	...
	metadataJSON, _ := json.Marshal(summary.Metadata) // ✅ FIX
...
func (r *attendanceRepository) GetCompanyAttendanceRules(
	...
	err := row.Scan(
		&rules.CompanyID,
		pq.Array(&allowedSources), // ✅ FIX: Postgres TEXT[]
...
func (r *attendanceRepository) UpsertCompanyAttendanceRules(
	...
	_, err := r.client.Exec(ctx, query,
		rules.CompanyID,
		pq.Array(rules.AllowedSourceTypes), // ✅ FIX
		rules.AllowMultipleCheckins,
		rules.Timezone,
		rules.CreatedAt,
	)
...
func (r *attendanceRepository) GetDepartmentAttendanceRules(
	...
	err := row.Scan(
		...
		&rules.CompanyID,
		&rules.DepartmentID,
		pq.Array(&allowedSources), // ✅ FIX
		pq.Array(&allowedEvents),  // ✅ FIX
...
func (r *attendanceRepository) UpsertDepartmentAttendanceRules(
	...
	_, err := r.client.Exec(ctx, query,
		...
		rules.DepartmentID,
		pq.Array(rules.AllowedSourceTypes), // ✅ FIX
		pq.Array(rules.AllowedEventTypes),  // ✅ FIX
		rules.RequireLocation,
		rules.RequireDevice,
		rules.CreatedAt,
	)
...
func (r *attendanceRepository) GetUserAttendanceProfile(
	...
	err := row.Scan(
		&profile.UserID,
		&profile.CompanyID,
		pq.Array(&overrideSources), // ✅ FIX
		pq.Array(&overrideEvents),  // ✅ FIX
		&profile.CreatedAt,
	)
...
func (r *attendanceRepository) UpsertUserAttendanceProfile(
	...
	_, err := r.client.Exec(ctx, query,
		profile.UserID,
		profile.CompanyID,
		pq.Array(profile.OverrideSourceTypes), // ✅ FIX
		pq.Array(profile.OverrideEventTypes),  // ✅ FIX
		profile.CreatedAt,
	)
	if err != nil {
		r.logger.Error(
...
func (r *attendanceRepository) CreateWorkCenterShift(
	ctx context.Context,
	...
	if err != nil {
		r.logger.Error("Failed to create work center shift",
			...
			util.String("work_center_code", mapping.WorkCenterCode),
			util.ErrorField(err))
...
func (r *attendanceRepository) GetWorkCenterShiftByCode(
	ctx context.Context,
	companyID uuid.UUID,
	workCenterCode string,
) (*attendance.WorkCenterShift, error) {
	query := `
        SELECT * FROM work_center_shifts 
        WHERE company_id = $1 
	...
	var mapping attendance.WorkCenterShift
	...
	if err != nil {
		...
		r.logger.Error("Failed to get work center shift",
			util.String("company_id", companyID.String()),
...
func (r *attendanceRepository) GetWorkCenterShiftsByCompany(
	ctx context.Context,
	companyID uuid.UUID,
	activeOnly bool,
) ([]*attendance.WorkCenterShift, error) {
	...
	rows, err := r.client.Query(ctx, query, companyID)
	if err != nil {
		r.logger.Error("Failed to get work center shifts",
			util.String("company_id", companyID.String()),
			util.ErrorField(err))
	...
	var mappings []*attendance.WorkCenterShift
	for rows.Next() {
		var mapping attendance.WorkCenterShift
		err := rows.Scan(
			&mapping.MappingID,
		...
		if err != nil {
			return nil, fmt.Errorf("failed to scan work center shift: %w", err)
		}
...
func (r *attendanceRepository) UpdateWorkCenterShift(
	ctx context.Context,
	mapping *attendance.WorkCenterShift,
) error {
	mapping.UpdatedAt = time.Now().UTC()
	...
	if err != nil {
		r.logger.Error("Failed to update work center shift",
			util.String("mapping_id", mapping.MappingID.String()),
			util.ErrorField(err))
	...
	rowsAffected, _ := result.RowsAffected()
	if rowsAffected == 0 {
		return fmt.Errorf("work center shift not found")
	}

	return nil
}

func (r *attendanceRepository) DeactivateWorkCenterShift(
	ctx context.Context,
	mappingID uuid.UUID,
) error {
	query := `
	...
	if err != nil {
		r.logger.Error("Failed to deactivate work center shift",
...
// GetLastAttendanceEvent gets the last attendance event of given type within time window
...
// func (r *attendanceRepository) GetSAPBusinessRules(
// 	ctx context.Context,
// 	companyID uuid.UUID,
// ) (*attendance.SAPBusinessRules, error) {
// 	// Note: SAP business rules table doesn't exist in provided schema
// 	// We'll implement when the table is created
// 	return nil, nil
// }

// func (r *attendanceRepository) UpdateSAPBusinessRules(
// 	ctx context.Context,
// 	companyID uuid.UUID,
