...
type EnrollmentRepository interface {
	...
	GetByIDUnsafe(ctx context.Context, db DBTX, id uuid.UUID) (*models.Enrollment, error)
...
func (r *enrollmentRepository) BulkCreate(ctx context.Context, db DBTX, enrollments []*models.Enrollment) error {
...
func (r *enrollmentRepository) GetByID(ctx context.Context, db DBTX, companyID, id uuid.UUID) (*models.Enrollment, error) {
	query := `
		...
		WHERE e.enrollment_id = $1 AND s.company_id = $2
	`
...
func (r *enrollmentRepository) GetByStudentAndYear(ctx context.Context, db DBTX, companyID, studentID, academicYearID uuid.UUID) (*models.Enrollment, error) {
	query := `
		...
		FROM academics.enrollments e
		JOIN academics.students s ON e.student_id = s.student_id
		WHERE e.student_id = $1 AND e.academic_year_id = $2 AND s.company_id = $3
	`
	row := db.QueryRowContext(ctx, query, studentID, academicYearID, companyID)
	return r.scanEnrollment(row)
}
...
func (r *enrollmentRepository) GetActiveByStudent(ctx context.Context, db DBTX, companyID, studentID uuid.UUID) (*models.Enrollment, error) {
	query := `
		...
		FROM academics.enrollments e
		JOIN academics.students s ON e.student_id = s.student_id
		WHERE e.student_id = $1 AND s.company_id = $2 AND e.status = $3
		LIMIT 1
	`
	row := db.QueryRowContext(ctx, query, studentID, companyID, EnrollmentStatusActive)
	return r.scanEnrollment(row)
}
...
func (r *enrollmentRepository) Update(ctx context.Context, db DBTX, e *models.Enrollment) error {
	query := `
		...
		SET section_id = $2, enrollment_date = $3, roll_number = $4,
		    ...
		    version = version + 1
		WHERE enrollment_id = $1 AND version = $7
...
func (r *enrollmentRepository) UpdateStatus(ctx context.Context, db DBTX, id uuid.UUID, status string, updatedBy *uuid.UUID) error {
	...
	if rows == 0 {
		return fmt.Errorf("enrollment %s not found", id)
	}
	return nil
}
...
func (r *enrollmentRepository) Delete(ctx context.Context, db DBTX, id uuid.UUID, deletedBy *uuid.UUID) error {
	query := `DELETE FROM academics.enrollments WHERE enrollment_id = $1`
...
func (r *enrollmentRepository) Exists(ctx context.Context, db DBTX, studentID, academicYearID uuid.UUID) (bool, error) {
	query := `SELECT EXISTS(SELECT 1 FROM academics.enrollments WHERE student_id = $1 AND academic_year_id = $2)`
	var exists bool
...
func (r *enrollmentRepository) CountBySection(ctx context.Context, db DBTX, sectionID uuid.UUID) (int64, error) {
	query := `SELECT COUNT(*) FROM academics.enrollments WHERE section_id = $1`
	var count int64
...
func (r *enrollmentRepository) CountByAcademicYear(ctx context.Context, db DBTX, academicYearID uuid.UUID) (int64, error) {
	query := `SELECT COUNT(*) FROM academics.enrollments WHERE academic_year_id = $1`
	var count int64
...
func (r *enrollmentRepository) CountActiveBySection(ctx context.Context, db DBTX, sectionID uuid.UUID) (int64, error) {
	query := `SELECT COUNT(*) FROM academics.enrollments WHERE section_id = $1 AND status = $2`
	var count int64
...
func (r *enrollmentRepository) CountActiveByStudent(ctx context.Context, db DBTX, studentID uuid.UUID) (int64, error) {
	query := `SELECT COUNT(*) FROM academics.enrollments WHERE student_id = $1 AND status = $2`
	var count int64
...
func (r *enrollmentRepository) CompleteActiveEnrollment(
	ctx context.Context,
	tx *sql.Tx,
	companyID, studentID, academicYearID uuid.UUID,
	updatedBy *uuid.UUID,
) error {
	query := `
...
func (r *enrollmentRepository) CompleteAllActiveEnrollments(
	ctx context.Context,
	tx *sql.Tx,
	companyID, studentID uuid.UUID,
	updatedBy *uuid.UUID,
) error {
	query := `
		UPDATE academics.enrollments e
...
func (r *enrollmentRepository) WithdrawAllActiveEnrollments(
	ctx context.Context,
	tx *sql.Tx,
	companyID, studentID uuid.UUID,
	updatedBy *uuid.UUID,
...
func (r *enrollmentRepository) GetActiveEnrollmentForUpdate(ctx context.Context, tx *sql.Tx, companyID, studentID, academicYearID uuid.UUID) (*models.Enrollment, error) {
	query := `
		...
		FOR UPDATE
	`
	row := tx.QueryRowContext(ctx, query, studentID, academicYearID, EnrollmentStatusActive, companyID)
	return r.scanEnrollment(row)
}
